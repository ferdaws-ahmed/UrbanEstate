...
export default function SellerChat() {
  ...
  const fetchMessages = async (contactId) => {
    if (!contactId) return;
    try {
      const res = await fetch(`/api/messages?receiverId=${contactId}`);
      const data = await res.json();
    ...
    try {
      const res = await fetch("/api/messages", {
        ...
        body: JSON.stringify({ 
          ...
          text: msgText,
          propertyId: activeChat.propertyId || null,
          propertyTitle: activeChat.propertyTitle || null
        }),
      });
      if (res.ok) {
  ...
  const handleCreateNewChat = async (e) => {
    ...
    try {
      // Find the user by email using the search API
      ...
      <div className={`w-full md:w-80 lg:w-96 flex flex-col border-r transition-all ${
        ...
        <div className="p-6 border-b border-inherit">
          <div className="flex items-center justify-between mb-4">
            <h2 className={`text-xl font-black ${isDark ? "text-white" : "text-slate-900"}`}>Client Messages</h2>
            <button 
              onClick={() => setIsNewChatModalOpen(true)}
              className="p-2 bg-teal-600 text-white rounded-xl hover:bg-teal-700 transition-colors shadow-lg shadow-teal-600/20"
              ...
              <button
                ...
                className={`w-full flex items-center gap-4 p-4 rounded-[1.5rem] transition-all mb-1 ${
                  activeChat?.id === contact.id
                    ? isDark ? "bg-teal-600/20 text-teal-400 shadow-inner" : "bg-teal-50 text-teal-600"
              ...
              >
                <div className="relative shrink-0">
                  ...
                  {contact.unreadCount > 0 && (
                    <span className="absolute -top-1 -right-1 w-5 h-5 bg-teal-600 text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-[var(--card)]">
                      {contact.unreadCount}
                    </span>
                  )}
                </div>
                <div className="flex-1 text-left min-w-0">
                  ...
                  {contact.propertyTitle && (
                    <p className="text-[10px] font-bold text-teal-600 truncate mb-0.5 uppercase tracking-wider">
                      {contact.propertyTitle}
                    </p>
                  )}
                  <p className={`text-[11px] truncate ${contact.unreadCount > 0 ? "font-black text-teal-500" : "text-slate-500"}`}>
      ...
      <div className={`flex-1 flex flex-col ${!activeChat ? "hidden md:flex items-center justify-center bg-slate-50/30 dark:bg-black/10" : "flex"}`}>
        {activeChat ? (
          <>
            ...
            <div className={`p-4 md:p-6 border-b flex items-center justify-between transition-all ${
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl overflow-hidden border-2 border-teal-600/20">
                    ...
                    <div key={i} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[80%] md:max-w-[70%] group relative`}>
                        ...
                        <div className={`flex items-center gap-2 mt-1.5 px-1 ${isMe ? "justify-end" : "justify-start"}`}>
                          ...
                          {isMe && <CheckCheck size={12} className={msg.seen ? "text-teal-500" : "text-slate-300"} />}
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div className={`p-6 transition-all ${isDark ? "bg-[var(--card)]" : "bg-white"}`}>
              <form onSubmit={handleSendMessage} className="flex items-center gap-4">
                ...
                <div className={`flex-1 flex items-center px-6 py-3 rounded-2xl border transition-all ${
                  isDark ? "bg-white/5 border-white/5 focus-within:border-teal-600/50" : "bg-slate-50 border-slate-100 focus-within:border-teal-600/50"
                }`}>
                  <input 
                    type="text" 
                    value={newMessage}
                ...
                <button 
                  ...
                  className="p-4 bg-teal-600 text-white rounded-2xl shadow-xl shadow-teal-600/20 hover:bg-teal-700 transition-all active:scale-95 disabled:opacity-50 disabled:pointer-events-none"
                >
                  <Send size={20} />
                </button>
              </form>
          ...
          <div className="text-center p-10">
            <div className="w-24 h-24 bg-teal-600/10 text-teal-600 rounded-[2rem] flex items-center justify-center mx-auto mb-6">
              <MessageCircle size={40} />
            </div>
            <h3 className={`text-2xl font-black mb-2 ${isDark ? "text-white" : "text-slate-900"}`}>Seller Messenger</h3>
            <p className="text-sm text-slate-500 max-w-xs mx-auto">Select a conversation to start chatting with clients.</p>
          </div>
        )}
      </div>
          ...
          <div className={`w-full max-w-md rounded-[2rem] p-6 shadow-2xl ${isDark ? 'bg-[var(--card)] border border-white/10' : 'bg-white border border-slate-200'}`}>
            ...
            <form onSubmit={handleCreateNewChat}>
                ...
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${
                      isDark 
                        ? 'bg-white/5 border-white/10 focus:border-teal-500 text-white' 
              ...
              <button 
                ...
                className="w-full py-3 bg-teal-600 text-white font-bold rounded-xl hover:bg-teal-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
