    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className={`p-8 rounded-[2.5rem] border transition-all hover:shadow-2xl hover:-translate-y-1 group ${
          ...
          <h3 className={`text-4xl font-black mb-1 ${isDark ? "text-white" : "text-slate-900"}`}>{stats.favorites}</h3>
          <p className={`text-[10px] font-black uppercase tracking-widest ${isDark ? "text-slate-500" : "text-slate-400"}`}>Saved Assets</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className={`p-8 rounded-[2.5rem] border ${
          ...
          <div className="flex items-center justify-between mb-8">
            <h3 className={`text-xl font-black tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}>Recent Favorites</h3>
              ...
              recentFavorites.map((fav) => (
                <div key={fav.id} className={`flex items-center gap-4 p-4 rounded-2xl transition-colors border ${
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className={`text-sm font-bold truncate ${isDark ? "text-white" : "text-slate-900"}`}>{fav.title}</h4>
        ...
        <div className={`p-8 rounded-[2.5rem] border ${
          ...
          <div className="flex items-center justify-between mb-8">
            <h3 className={`text-xl font-black tracking-tight ${isDark ? "text-white" : "text-slate-900"}`}>My Inquiries</h3>
                ...
                return (
                  <div key={inquiry.id} className={`flex gap-4 p-5 rounded-2xl border transition-all hover:shadow-lg hover:-translate-y-1 ${
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h4 className={`text-xs font-bold mb-1 truncate ${isDark ? "text-white" : "text-slate-900"}`}>{inquiry.propertyTitle}</h4>
                      <p className={`text-[11px] truncate italic ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>"{inquiry.message}"</p>
                    </div>
                  </div>
                );
              })
      ...
      <Link
        ...
        className={`fixed bottom-10 right-10 z-[100] flex h-16 w-16 items-center justify-center rounded-[2rem] shadow-2xl transition-all duration-500 hover:scale-110 active:scale-95 group
          ${isDark ? "bg-[#1a4a40] border border-white/10" : "bg-blue-600 border border-blue-500"}
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute -top-2 -right-2 h-7 w-7 flex items-center justify-center bg-red-500 text-white text-[10px] font-black rounded-full border-2 border-white dark:border-[#0b1f1a] shadow-lg animate-in zoom-in duration-300">
            {unreadCount}
          </span>
        )}
