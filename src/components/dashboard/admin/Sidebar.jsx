...
import {
  ...
  Settings, LifeBuoy, HelpCircle, X, Lock, Unlock, MessageSquare, ShieldAlert
} from 'lucide-react'; 

const bgImages = {
  default: "https://i.ibb.co/gZz1V6Ck/1.png",
  property: "https://i.ibb.co/RnDgvKJ/2.png",
  users: "https://i.ibb.co/Y7S2Pyfp/3.png"
};
...
export default function Sidebar({ isOpen, setIsOpen, isCollapsed, setIsCollapsed }) { 
  ...
  const menuItems = [
    { name: 'Overview', path: '/dashboard/admin', icon: <LayoutDashboard size={22} /> },
    { name: 'Properties', path: '/dashboard/admin/properties', icon: <Home size={22} /> },
    { name: 'Reports', path: '/dashboard/admin/reports', icon: <ShieldAlert size={22} />, badge: unreadCount },
    { name: 'Sellers', path: '/dashboard/admin/sellers', icon: <Users size={22} /> },
    { name: 'Users', path: '/dashboard/admin/users', icon: <UserCircle size={22} /> },
    { name: 'Analytics', path: '/dashboard/admin/analytics', icon: <BarChart3 size={22} /> },
    { name: 'Chat', path: '/dashboard/admin/chat', icon: <MessageSquare size={22} />, badge: unreadChatCount },
    { name: 'Settings', path: '/dashboard/admin/settings', icon: <Settings size={22} /> },
    { name: 'Support', path: '/dashboard/admin/support', icon: <LifeBuoy size={22} /> },
    { name: 'Help', path: '/dashboard/admin/help', icon: <HelpCircle size={22} /> },
  ];
      ...
      `}>
        ...
        <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
          {Object.entries(bgImages).map(([key, url]) => {
            const isVisible = (key === 'property' && pathname.includes('/properties')) || 
                              (key === 'users' && pathname.includes('/users')) || 
                              (key === 'default' && !pathname.includes('/properties') && !pathname.includes('/users'));
            return (
              <div 
                key={key}
                ...
                return (
                  <li key={index}>
                    ...
                    >
                      ...
                      {item.badge > 0 && (
                        <span className={`ml-auto flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] font-bold text-white shadow-lg transition-all duration-300 ${isCollapsed ? 'absolute top-2 right-2 scale-75' : 'scale-100'}`}>
                          {item.badge}
                        </span>
                      )}
