...
export default function MiniAgentPerformance() {
  const { isDark } = useTheme();


  const agents = [
    { id: 1, name: 'S. Mr Islam', role: 'Senior Agent', sales: 45, avatar: 'S+Islam' },
    { id: 2, name: 'Agent Naron', role: 'Property Consultant', sales: 38, avatar: 'Agent+Naron' },
    { id: 3, name: 'Sarah Johnson', role: 'Luxury Specialist', sales: 32, avatar: 'Sarah+J' },
    { id: 4, name: 'David Martinez', role: 'Commercial Agent', sales: 28, avatar: 'David+M' },
    { id: 5, name: 'Michael Chen', role: 'Investment Advisor', sales: 24, avatar: 'Michael+C' },
    { id: 6, name: 'Aisha Rahman', role: 'Leasing Agent', sales: 19, avatar: 'Aisha+R' },
    { id: 7, name: 'Hasan Mahmud', role: 'Area Manager', sales: 17, avatar: 'Hasan+M' },
    { id: 8, name: 'Emily Clark', role: 'Junior Agent', sales: 15, avatar: 'Emily+C' },
    { id: 9, name: 'Tariq Zia', role: 'Field Executive', sales: 12, avatar: 'Tariq+Z' },
    { id: 10, name: 'Fatima Begum', role: 'Client Advisor', sales: 10, avatar: 'Fatima+B' },
  ...
  return (
    <div className={`p-3 sm:p-4 rounded-[1.5rem] sm:rounded-[2rem] border backdrop-blur-xl transition-all duration-500 shadow-xl h-full flex flex-col overflow-hidden relative ${
      ...
      {/* Agent List with Custom Scrollbar */}
      <div className={`flex-1 overflow-y-auto overflow-x-hidden pr-1 pb-1 custom-scrollbar relative z-10 ${
        ...
        <div className="flex flex-col gap-1.5 w-full">
          {agents.map((agent, index) => (
            ...
            <div key={agent.id} className={`flex items-center justify-between gap-1 p-1.5 sm:p-2 rounded-xl transition-all duration-300 group ${
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-1">
                ...
                <div className="w-3 sm:w-3.5 shrink-0 flex justify-center">
                  {index === 0 ? (
                    ...
                    <span className={`text-[8px] sm:text-[9px] font-bold ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{agent.id}</span>
                  )}
                </div>
                
                {/* Avatar */}
                <img 
                  src={`https://ui-avatars.com/api/?name=${agent.avatar}&background=random&color=fff&bold=true`} 
                  alt={agent.name} 
