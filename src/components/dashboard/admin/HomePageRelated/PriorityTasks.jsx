...
export default function PriorityTasks() {
  ...
  return (
    ...
    <div className={`p-4 sm:p-5 rounded-[1.5rem] sm:rounded-[2rem] border backdrop-blur-xl transition-all duration-500 shadow-xl flex flex-col overflow-hidden relative h-[380px] sm:h-[400px] w-full ${
      isDark ? 'bg-gradient-to-b from-[var(--card)]/80 to-[var(--background)] border-white/10 shadow-black/40 text-white' : 'bg-white/80 border-gray-200 shadow-gray-200/50 text-gray-900'
    }`}>
          ...
          {tasks.map((task) => (
            <div key={task.id} className={`flex items-start gap-3 p-3 rounded-xl transition-all duration-300 border group cursor-pointer ${
              ...
              <div className={`mt-0.5 w-4 h-4 sm:w-5 sm:h-5 rounded flex items-center justify-center shrink-0 transition-all duration-300 border ${
                task.isCompleted ? (isDark ? 'bg-[#cddfa0] border-[#cddfa0] text-[var(--card)]' : 'bg-emerald-500 border-emerald-500 text-white') : (isDark ? 'bg-[var(--background)] border-white/10 group-hover:border-[#cddfa0]' : 'bg-gray-50 border-gray-300 group-hover:border-emerald-400')
              }`}>
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center">
                ...
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  <div className={`flex items-center gap-1 text-[7px] sm:text-[8px] font-bold px-1.5 py-0.5 rounded-sm ${isDark ? 'bg-[var(--card)] text-gray-400' : 'bg-gray-100 text-gray-500'}`}>
