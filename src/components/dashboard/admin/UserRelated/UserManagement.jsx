...
export default function UserManagement() {
  ...
  const handleUpdateStatus = async (id, newStatus) => {
    ...
    try {
      ...
      toast.success(`User status updated to ${newStatus}`, { id: tid });
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        ...
        <p className={`text-sm font-bold uppercase tracking-widest ${isDark ? 'text-white' : 'text-gray-800'}`}>Synchronizing Users...</p>
      </div>
    );
  }
    ...
    <div className="w-full">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className={`text-3xl font-black tracking-tight flex items-center gap-3 ${isDark ? 'text-white' : 'text-slate-900'}`}>
            ...
            User <span className="text-emerald-500">Database</span>
          </h1>
          <p className="text-sm font-bold text-gray-500 uppercase tracking-[0.2em] mt-2">Managing {users.length} Registered Users</p>
        </div>
      ...
      {viewMode === 'table' ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className={`border-b ${isDark ? 'border-white/10 bg-black/20' : 'border-gray-100 bg-gray-50'}`}>
                  <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-500">User Details</th>
                  <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-500">Contact Info</th>
                  <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-500 text-center">Activity</th>
              ...
              <tbody className="divide-y divide-transparent">
                {paginatedUsers.map((u) => (
                  <tr key={u.id} className={`group transition-all duration-300 ${isDark ? 'hover:bg-white/5' : 'hover:bg-gray-50'} ${u.status === 'Terminated' ? 'opacity-60 grayscale' : ''}`}>
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          <img src={u.avatar} alt={u.name} className="w-12 h-12 rounded-2xl object-cover border-2 border-emerald-500/20" />
                          <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 ${isDark ? 'border-[var(--card)]' : 'border-white'} ${
                            u.status === 'Active' ? 'bg-emerald-500' : u.status === 'Frozen' ? 'bg-blue-500' : 'bg-red-500'
                          }`}></div>
                        </div>
                        <div>
                          <p className={`text-sm font-black ${isDark ? 'text-white' : 'text-gray-900'}`}>{u.name}</p>
                          <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest mt-0.5">Registered User</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2 text-[11px] font-medium text-gray-500">
                          <Mail size={12} className="text-emerald-500" /> {u.email}
                        </div>
                        <div className="flex items-center gap-2 text-[11px] font-medium text-gray-500">
                    ...
                    <td className="px-8 py-5 text-center">
                      <div className="inline-flex flex-col items-center">
                        <p className={`text-lg font-black ${isDark ? 'text-[#cddfa0]' : 'text-emerald-700'}`}>{new Date(u.createdAt).toLocaleDateString()}</p>
                        <p className="text-[9px] font-black uppercase text-gray-500 tracking-tighter">Join Date</p>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex justify-center">
                        <span className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest ${
                          u.status === 'Active' ? (isDark ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-100 text-emerald-700') :
                          u.status === 'Frozen' ? (isDark ? 'bg-blue-500/10 text-blue-400' : 'bg-blue-100 text-blue-700') :
                    ...
                    <td className="px-8 py-5 text-right">
                      <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                        {u.status !== 'Active' && (
                          <button onClick={() => handleUpdateStatus(u.id, 'Active')} className={`p-2.5 rounded-xl transition-all ${isDark ? 'bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500 hover:text-white' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white'}`} title="Activate User">
                            <ShieldCheck size={18} />
                          </button>
                        )}
                        {u.status === 'Active' && (
                          <button onClick={() => handleUpdateStatus(u.id, 'Frozen')} className={`p-2.5 rounded-xl transition-all ${isDark ? 'bg-blue-500/10 text-blue-400 hover:bg-blue-500 hover:text-white' : 'bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white'}`} title="Freeze Account">
                            <Snowflake size={18} />
                          </button>
                        )}
                        <button onClick={() => handleUpdateStatus(u.id, 'Terminated')} className={`p-2.5 rounded-xl transition-all ${isDark ? 'bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white' : 'bg-red-50 text-red-600 hover:bg-red-600 hover:text-white'}`} title="Terminate Account">
                          <UserMinus size={18} />
                        </button>
                      </div>
                    </td>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {paginatedUsers.map((u) => (
            <div key={u.id} className={`group rounded-[2.5rem] border overflow-hidden transition-all duration-500 hover:scale-[1.02] relative ${
              isDark ? 'bg-[var(--card)]/40 border-white/10 hover:bg-[var(--card)]' : 'bg-white border-gray-200 shadow-lg shadow-gray-200/20'
            } ${u.status === 'Terminated' ? 'opacity-60 grayscale' : ''}`}>
              <div className="p-6">
                <div className="flex items-start justify-between mb-6">
                  <div className="relative">
                    <img src={u.avatar} alt={u.name} className="w-16 h-16 rounded-[1.5rem] object-cover border-4 border-emerald-500/10 shadow-xl" />
                  ...
                  <div className="text-right">
                    <span className={`px-3 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-widest ${
                      u.status === 'Active' ? 'bg-emerald-500 text-white' : u.status === 'Frozen' ? 'bg-blue-500 text-white' : 'bg-red-500 text-white'
                    }`}>
                      {u.status}
                    </span>
                  </div>
                ...
                <h3 className={`text-lg font-black truncate mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>{u.name}</h3>
                <p className="text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-4">Registered User</p>
                ...
                <div className={`space-y-2 mb-6 p-4 rounded-2xl ${isDark ? 'bg-black/20' : 'bg-gray-50'}`}>
                  <div className="flex items-center gap-3 text-[11px] font-bold text-gray-500">
                    <Mail size={14} className="text-emerald-500 shrink-0" /> <span className="truncate">{u.email}</span>
                  </div>
                  <div className="flex items-center gap-3 text-[11px] font-bold text-gray-500">
                    <Phone size={14} className="text-emerald-500 shrink-0" /> {u.phone}
                  </div>
                </div>

                <div className="flex gap-2">
                  {u.status !== 'Active' && (
                    <button onClick={() => handleUpdateStatus(u.id, 'Active')} className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${isDark ? 'bg-emerald-500 text-[var(--background)]' : 'bg-emerald-600 text-white'}`}>
                      <ShieldCheck size={14} /> Activate
                    </button>
                  )}
                  {u.status === 'Active' && (
                    <button onClick={() => handleUpdateStatus(u.id, 'Frozen')} className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${isDark ? 'bg-blue-500 text-white' : 'bg-blue-600 text-white'}`}>
                      <Snowflake size={14} /> Freeze
                    </button>
                  )}
                  <button onClick={() => handleUpdateStatus(u.id, 'Terminated')} className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${isDark ? 'bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-white' : 'bg-red-50 text-red-600 border border-red-100 hover:bg-red-600 hover:text-white'}`}>
