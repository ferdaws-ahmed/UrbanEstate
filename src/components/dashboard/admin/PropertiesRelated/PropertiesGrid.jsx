...
export default function PropertiesGrid() {
  ...
  if (!mounted || isLoading) {
    return (
      <div className={`min-h-screen w-full flex flex-col items-center justify-center ${isDark ? 'bg-[var(--background)]' : 'bg-gray-50'}`}>
        <div className="relative">
          <div className={`w-16 h-16 border-4 rounded-full ${isDark ? 'border-white/10' : 'border-gray-200'}`}></div>
  ...
  return (
    <div className="relative p-4 md:p-6 transition-colors duration-500 bg-gray-50 dark:bg-[var(--background)] text-gray-900 dark:text-gray-100 min-h-screen overflow-hidden">
      
      {/* Background Ambience */}
      <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-[#cddfa0]/10 rounded-full blur-[100px] pointer-events-none"></div>
      <div className="absolute bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-[var(--card)]/30 rounded-full blur-[100px] pointer-events-none"></div>
      ...
      <div className={`fixed top-5 left-1/2 transform -translate-x-1/2 z-[70] transition-all duration-300 ${toast.show ? 'translate-y-0 opacity-100' : '-translate-y-20 opacity-0 pointer-events-none'}`}>
        <div className="bg-emerald-500 dark:bg-[#cddfa0] text-white dark:text-[var(--background)] px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 font-bold">
          <CheckCircle size={20} />
          {toast.message}
        </div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        {/* HEADER SECTION*/}
        <div className="mb-8 flex flex-col lg:flex-row lg:items-end justify-between gap-6 pb-6 border-b border-gray-200 dark:border-white/10">
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              <span className="inline-flex items-center justify-center px-4 py-1.5 rounded-full bg-white dark:bg-[var(--card)] text-gray-800 dark:text-[#cddfa0] text-[11px] font-bold uppercase tracking-widest border border-gray-200 dark:border-white/10 shadow-sm">
                Admin Control Panel
              </span>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6">
            <div className="flex w-full sm:w-auto items-center gap-6 bg-white dark:bg-[var(--card)]/40 p-4 rounded-2xl border border-gray-100 dark:border-white/10 shadow-sm backdrop-blur-md">
              <div className="pl-2 pr-6 border-r border-gray-200 dark:border-white/10">
              ...
              <div className="pr-4 flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-blue-50 dark:bg-[var(--card)] flex items-center justify-center">
        ...
        <div className="flex flex-col lg:flex-row gap-3 mb-8">
          <div className="relative w-full lg:max-w-[750px] group">
            <div className={`absolute -inset-0.5 rounded-lg blur opacity-0 group-hover:opacity-100 transition duration-500 ${isDark ? 'bg-gradient-to-r from-[#cddfa0]/40 to-[var(--card)]/40' : 'bg-gradient-to-r from-blue-200 to-indigo-200'}`}></div>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 rounded-lg outline-none transition-all duration-300 bg-white/90 dark:bg-[var(--card)]/50 backdrop-blur-md border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white placeholder-gray-400 focus:border-[#cddfa0] focus:ring-2 focus:ring-[#cddfa0]/20 shadow-sm text-sm"
              />
            </div>
          </div>
          ...
          <div className="flex gap-2 flex-wrap md:flex-nowrap">
            <select
              ...
              className="flex-1 md:w-auto px-3 py-2.5 rounded-lg outline-none cursor-pointer transition-all duration-300 bg-white/90 dark:bg-[var(--card)]/50 backdrop-blur-md border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:border-[#cddfa0] focus:ring-2 focus:ring-[#cddfa0]/20 shadow-sm text-sm"
            >
              {propertyTypes.map(type => (
                <option key={type} value={type} className="bg-white dark:bg-[var(--primary)] text-gray-900 dark:text-white">{type}</option>
              ))}
            </select>
            ...
            >
              {statusTypes.map(status => (
                <option key={status} value={status} className="bg-white dark:bg-[var(--primary)] text-gray-900 dark:text-white">{status}</option>
              ))}
            </select>
            ...
            <select
              ...
              className="flex-1 md:w-auto px-3 py-2.5 rounded-lg outline-none cursor-pointer transition-all duration-300 bg-white/90 dark:bg-[var(--card)]/50 backdrop-blur-md border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white focus:border-[#cddfa0] focus:ring-2 focus:ring-[#cddfa0]/20 shadow-sm text-sm"
            >
              <option value="default" className="bg-white dark:bg-[var(--primary)] text-gray-900 dark:text-white">Sort by Price</option>
              <option value="low-high" className="bg-white dark:bg-[var(--primary)] text-gray-900 dark:text-white">Price: Low to High</option>
              <option value="high-low" className="bg-white dark:bg-[var(--primary)] text-gray-900 dark:text-white">Price: High to Low</option>
            </select>

            <div className="flex items-center gap-1 px-2 py-1.5 rounded-lg bg-white/90 dark:bg-[var(--card)]/50 backdrop-blur-md border border-gray-200 dark:border-white/10 shadow-sm text-sm">
              <input
                type="number"
                placeholder="Min"
                value={minPrice}
              ...
              />
              <span className="text-gray-300 dark:text-[var(--card)] font-bold">|</span>
              <input
                type="number"
                placeholder="Max"
                value={maxPrice}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {currentItems.map((property) => (
            <div 
              key={property.id} 
              className="group relative rounded-2xl overflow-hidden transition-all duration-500 bg-white dark:bg-[var(--card)]/40 backdrop-blur-xl border border-gray-100 dark:border-[var(--card)]/60 shadow-lg hover:-translate-y-2 hover:shadow-2xl dark:hover:shadow-[#cddfa0]/10 hover:border-[#cddfa0]/50 flex flex-col"
            >
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <div className="flex justify-between text-xs text-gray-700 dark:text-gray-300 mb-4 border-y border-gray-100 dark:border-white/10 py-2.5">
                ...
                <div className="mt-auto flex justify-between items-center pt-1">
                  ...
                  <div className="flex gap-1.5">
                    <Link href={`/property/${property.id}`} className="p-2 bg-gray-100 dark:bg-[var(--card)]/60 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-[var(--card)] transition-colors"><Eye size={16} /></Link>
        ...
        {filteredProperties.length === 0 && (
          <div className="text-center py-20 relative">
            <Search size={40} className="mx-auto mb-3 text-gray-300 dark:text-[var(--card)]" />
      ...
      {deleteModal.isOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[var(--background)] border border-gray-100 dark:border-[var(--card)]/60 rounded-2xl p-6 w-full max-w-sm shadow-2xl relative">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteModal({ isOpen: false, id: null })} className="flex-1 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 font-bold hover:bg-gray-50 dark:hover:bg-[var(--card)] transition-colors">Cancel</button>
      ...
      {formModal.isOpen && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[var(--background)] border border-gray-100 dark:border-[var(--card)]/60 rounded-2xl p-6 w-full max-w-md shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <button onClick={() => setFormModal({ isOpen: false, type: 'ADD', property: null })} className="absolute top-4 right-4 p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 dark:hover:bg-[var(--card)] transition-colors"><X size={20} /></button>
            
            <div className="flex items-center gap-3 mb-6">
              <div className="w-10 h-10 rounded-full bg-blue-50 dark:bg-[var(--card)] flex items-center justify-center text-blue-600 dark:text-[#cddfa0]">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              <div>
                ...
                <input name="title" required type="text" defaultValue={formModal.property?.title} placeholder="e.g. Modern Villa" className="w-full px-4 py-2.5 rounded-xl bg-gray-50 dark:bg-[var(--card)]/50 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white text-sm outline-none focus:border-[#cddfa0] focus:ring-2 focus:ring-[#cddfa0]/20 transition-all" />
              </div>
              ...
              <div>
                ...
                <input name="image" type="url" defaultValue={formModal.property?.image} placeholder="https://example.com/image.jpg" className="w-full px-4 py-2.5 rounded-xl bg-gray-50 dark:bg-[var(--card)]/50 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white text-sm outline-none focus:border-[#cddfa0] focus:ring-2 focus:ring-[#cddfa0]/20 transition-all" />
              </div>
              ...
              <div>
                ...
                <input name="location" required type="text" defaultValue={formModal.property?.location} placeholder="e.g. Dhaka, Bangladesh" className="w-full px-4 py-2.5 rounded-xl bg-gray-50 dark:bg-[var(--card)]/50 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white text-sm outline-none focus:border-[#cddfa0] focus:ring-2 focus:ring-[#cddfa0]/20 transition-all" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input name="price" required type="number" defaultValue={formModal.property?.price} placeholder="e.g. 500000" className="w-full px-4 py-2.5 rounded-xl bg-gray-50 dark:bg-[var(--card)]/50 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white text-sm outline-none focus:border-[#cddfa0] focus:ring-2 focus:ring-[#cddfa0]/20 transition-all" />
                </div>
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-1.5">Status</label>
                  <select name="status" defaultValue={formModal.property?.status || 'Available'} className="w-full px-4 py-2.5 rounded-xl bg-gray-50 dark:bg-[var(--card)]/50 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white text-sm outline-none focus:border-[#cddfa0] focus:ring-2 focus:ring-[#cddfa0]/20 transition-all">
                ...
                {formModal.type === 'ADD' && (
                  <>
                    <div>
                      ...
                      <input name="beds" type="number" placeholder="3" className="w-full px-4 py-2.5 rounded-xl bg-gray-50 dark:bg-[var(--card)]/50 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white text-sm outline-none focus:border-[#cddfa0] focus:ring-2 focus:ring-[#cddfa0]/20 transition-all" />
                    </div>
                    <div>
                      <label className="block text-[11px] font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-1.5">Baths</label>
                      <input name="baths" type="number" placeholder="2" className="w-full px-4 py-2.5 rounded-xl bg-gray-50 dark:bg-[var(--card)]/50 border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white text-sm outline-none focus:border-[#cddfa0] focus:ring-2 focus:ring-[#cddfa0]/20 transition-all" />
                    </div>
                  </>
                )}
              </div>
              ...
              <button 
                type="submit" 
                className="relative group overflow-hidden w-full py-3.5 mt-4 bg-gradient-to-r from-blue-600 to-indigo-600 dark:from-[#cddfa0] dark:to-[#aebf85] text-white dark:text-[var(--background)] rounded-xl font-black transition-all duration-300 shadow-[0_8px_20px_rgb(37,99,235,0.3)] dark:shadow-[0_8px_20px_rgba(205,223,160,0.3)] hover:shadow-[0_8px_30px_rgb(37,99,235,0.5)] dark:hover:shadow-[0_8px_30px_rgba(205,223,160,0.5)] hover:-translate-y-1"
              >
                <div className="absolute inset-0 w-full h-full bg-white/20 dark:bg-black/10 transform -skew-x-12 -translate-x-full group-hover:animate-shine z-0"></div>
