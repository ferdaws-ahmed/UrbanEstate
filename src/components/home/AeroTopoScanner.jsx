...
const scanModes = [
  {
    ...
    // Topography/Landscape Analysis Image
  ...
  },
  {
    ...
    icon: <Wind size={18} />,
    // Airflow/Wind Simulation Visual Image
  ...
  },
  {
    ...
    // Soil/Geological Structure Image
...
export default function AeroTopoScanner() {
  ...
  return (
    <section className={`w-full py-24 px-6 lg:px-12 bg-[#081d19] relative overflow-hidden ${manrope.className}`}>
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="text-4xl lg:text-6xl font-black text-white mb-6 tracking-tight leading-[1.1]">
            Aero-Topo & <span className="text-[#cddfa0] italic font-light">Micro-Climate Scanner</span>
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                ...
                <button
                  ...
                  className={`flex items-center justify-between p-5 rounded-2xl border transition-all duration-500 text-left group ${
                    activeMode.id === mode.id
                      ? "bg-[#cddfa0] border-[#cddfa0] shadow-[0_10px_30px_rgba(205,223,160,0.2)]"
                      : "bg-white/5 border-white/10 hover:border-[#cddfa0]/50"
                  }`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`p-2 rounded-lg transition-colors ${activeMode.id === mode.id ? 'bg-[#081d19] text-[#cddfa0]' : 'bg-white/10 text-[#cddfa0]'}`}>
                      {mode.icon}
                    </div>
                    <span className={`font-bold text-sm tracking-wide ${activeMode.id === mode.id ? 'text-[#081d19]' : 'text-white/80 group-hover:text-white'}`}>
                      {mode.label}
                    </span>
                  </div>
                  <ArrowUpRight size={16} className={`transition-transform duration-500 ${activeMode.id === mode.id ? 'text-[#081d19] rotate-45' : 'text-white/20'}`} />
                </button>
              ))}
            </div>

            {/* Readout Box */}
            <div className="bg-[#0f2e28] border border-white/10 rounded-3xl p-8 relative overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                {Object.entries(activeMode.data).map(([key, value]) => (
                  <div key={key} className="bg-black/20 p-4 rounded-xl border border-white/5">
                    <p className="text-white/30 text-[8px] uppercase font-black mb-1">{key}</p>
          ...
          <div className="lg:col-span-8 relative aspect-video lg:aspect-auto h-[450px] lg:h-[650px] rounded-[3rem] overflow-hidden border border-white/10 shadow-2xl bg-[#040f0c] group">
            
            {/* Dynamic Background Image - Mode অনুযায়ী পরিবর্তন হবে */}
            <div className="absolute inset-0 transition-all duration-1000 transform group-hover:scale-105">
              <img 
                ...
                className="w-full h-full object-cover opacity-50 grayscale mix-blend-luminosity animate-in fade-in duration-700"
              />
              <div className="absolute inset-0 bg-[#081d19] mix-blend-color opacity-60"></div>
            </div>

            {/* Technical HUD Overlay */}
            <div className="absolute inset-0 z-20 pointer-events-none">
              {/* Radar Sweep Animation */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[180%] h-[1px] bg-[#cddfa0]/20 origin-center animate-[spin_8s_linear_infinite]"></div>
              
              {/* Scanning Laser Line */}
              <div className="absolute w-full h-[2px] bg-gradient-to-r from-transparent via-[#cddfa0]/50 to-transparent shadow-[0_0_15px_#cddfa0] top-0 animate-[scan_4s_ease-in-out_infinite]"></div>
              
              {/* HUD Text Elements */}
              <div className="absolute top-8 left-10 flex flex-col gap-1">
                <span className="text-[#cddfa0] font-mono text-[10px] tracking-[0.2em] uppercase">Status: Analyzing Environment</span>
              ...
              {/* Dynamic Logic Visuals based on Mode */}
              <div className="absolute inset-0 flex items-center justify-center">
                ...
                {activeMode.id === 'wind' && (
                  <div className="grid grid-cols-6 gap-20 opacity-40">
                    {[...Array(12)].map((_, i) => (
                ...
                {activeMode.id === 'stability' && (
                  <div className="relative">
                    <div className="w-48 h-48 border-2 border-[#cddfa0]/20 rounded-full animate-ping"></div>
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                      <ShieldCheck size={80} className="text-[#cddfa0] drop-shadow-[0_0_15px_#cddfa0]" strokeWidth={1} />
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* Bottom Tag */}
            <div className="absolute bottom-10 left-10 z-30 flex items-center gap-3 bg-black/50 backdrop-blur-md px-6 py-3 rounded-full border border-white/10">
