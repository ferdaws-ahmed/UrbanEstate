...
const PropertyLocation = ({ propertyLocation, setPropertyLocation }) => {
  const [position, setPosition] = useState([
    propertyLocation?.coordinates?.coordinates[1] || 90.4125,
    propertyLocation?.coordinates?.coordinates[0] || 23.8103,
  ]);
  const [searchQuery, setSearchQuery] = useState("");
  const [addressDetails, setAddressDetails] = useState(
    propertyLocation?.formattedAddress || "",
  );
  useEffect(() => {
    // Update parent property.location whenever local position or address changes
    if (setPropertyLocation) {
      setPropertyLocation({
        lat: position[0],
        lng: position[1],
    ...
    <div className="p-6 md:p-8 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700">
      <div className="mb-6">
        ...
        <p className="text-slate-500 dark:text-slate-400">
          Search an address or click on the map to pin the location.
        </p>
      </div>
